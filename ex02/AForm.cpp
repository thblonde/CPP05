...
AForm::AForm(std::string const name, bool isSigned, int const gradeToSign, int const gradeToExecute)
...
void AForm::execute(Bureaucrat const & executor) const
{
    ...
    {
         std::cout << "Bureaucrat " << executor.getName() << " executed form "
            << this->_name << std::endl;
        this->executeForm();
    }
    else
    {
        std::cout << "Bureaucrat " << executor.getName() << " couldn't execute form "
            << this->_name << " because ";
        throw GradeTooLowException();
    }
}
